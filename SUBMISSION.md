...
GatemanAI is built for the half of the world flagship AI ignores. Lagos, Nairobi, Dhaka, places with WhatsApp-shaped infrastructure, $50-phone realities, and intermittent power and internet. Every architectural choice in this project comes from constraints those places impose.
 
Cloud AI fails without monthly plans. On-device flagship AI fails on mid-tier phones. Apps don't get installed when data and storage are expensive. Internet drops; hardware budgets sit near $50.
 
Every choice here follows: WhatsApp instead of a custom app, WhatsApp Cloud API for distribution, local Gemma on an edge device already in the home, a model variant tuned for 8GB-RAM consumer hardware, a $35 Pi 3 B+ as the prototype sensor with a clean upgrade path to a single Raspberry Pi 5 or better in production.
...
Around Yabatech Polytechnic in Lagos, gun violence is a painful reality. The school's gate was locked down for over a year, only to see another incident shortly after reopening. A bakery manager i spoke with described the daily fear of walking around the back gate. Armed forces guard the school; civilians around it have no protection. CCTV and smart cameras are either too expensive or require infrastructure outside their reality.
...
**Pull.** Users message "who is outside?" or "what is going on" for a live frame, or "what happened in the last hour?" for a retrospective. Gemma routes between three MCP tools (live capture, recent-event listing, and photo retrieval by event ID) to either grab a fresh frame or browse the past 7 days.
 
### MCP over WhatsApp
 
The pull flow reveals a useful pattern: with an MCP-speaking agent and a WhatsApp gateway, any function-callable tool becomes reachable from any phone through an interface people already know. No new app, no account creation.
...
GatemanAI becomes the alerting and accessibility layer on top of cameras people already own; existing recording keeps running while we add the WhatsApp integration, the AI description, and the retrospective the existing app cannot answer.
...
The original plan ran everything on the Pi. Gemma 4 E2B at 4-bit quantisation needs ~1.5GB of weights; the Pi 3 B+ has 1GB. The model wouldn't load, forcing the split: Pi as the body, laptop on the same Wi-Fi as the brain. Most downstream choices (the long-poll, the MCP integration, the 8k Modelfile) follow from it. A Pi 5 8GB collapses the hybrid back to one device in production.
...
The prototype uses OpenClaw to skip API approval delays during the hackathon build. Production migrates to the official Meta WhatsApp Cloud API; the Go architecture, Gemma integration, and MCP tooling are designed to stay unchanged across the swap.
...
What's next: multi-camera deployments (a small shop with cameras on entrance, till, back exit), richer event classification beyond pixel diff (loitering, vehicle stopped, unfamiliar face), a two-tier alert separating the fast motion ping from the slower scene description, and outdoor enclosures with PoE or solar.
